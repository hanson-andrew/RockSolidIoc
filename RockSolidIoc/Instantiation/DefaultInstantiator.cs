...
{
  internal class DefaultInstantiator : IInstantiator
  {
    ...
    public object ResolveDependency(Type type, IIocContainer context)
    {
      ...
      {
        object[] attributes = prop.GetCustomAttributes(typeof(IocDependencyAttribute), true);
        if (attributes.Length > 0)
        {
          IocDependencyAttribute dependencyAttribute = (attributes[0] as IocDependencyAttribute);
      ...
      {
        object[] attributes = method.GetCustomAttributes(typeof(IocDependencyAttribute), true);
        if (attributes.Length > 0)
        {
    ...
    private object[] ResolveParameters(MethodBase methodBase, IIocContainer context)
    {
      ...
      for (int paramIndexer = 0; paramIndexer < methodParams.Length; paramIndexer++)
      {
        ParameterInfo param = (ParameterInfo)methodParams.GetValue(paramIndexer);
        try
        {
          string identString = string.Empty;
          object[] attrs = param.GetCustomAttributes(typeof(IocDependencyAttribute), false);
